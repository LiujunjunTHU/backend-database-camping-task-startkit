...
select count(*) as 用戶數量_含教練 from "USER";
...
select * from "COURSE_BOOKING" where 
	user_id = (select id from "USER" where email = 'wXlTq@hexschooltest.io');
...
select "USER".name, "USER".id,
	coalesce("CREDIT_PURCHASE".total_credits, 0) - coalesce("COURSE_BOOKING".total_courses, 0)
	as remaining_credit
from "USER"
inner join(
	select user_id, sum(purchased_credits) as total_credits
	from "CREDIT_PURCHASE" group by user_id
) "CREDIT_PURCHASE" on "USER".id = "CREDIT_PURCHASE".user_id
inner join(
	select user_id, count(*) as total_courses from "COURSE_BOOKING"
	where status != '課程已取消' group by user_id
...
where
----	12月開始做功課，資料內存檔為12月，下列註解為測試用
--	"CREDIT_PURCHASE".purchase_at >= '2024-12-01 00:00:00'
